...
static void perf_pick_instrument(void)
{
    ...
    while (!done) {
        ...
        for (int i=0;i<nf;i++) {
            ...
            int ok=!query[0];
            if (!ok) { int ql=(int)strlen(query),hl=(int)strlen(nm);
                for (int j=0;j<=hl-ql&&!ok;j++) { int m=1;
                    for (int k=0;k<ql&&m;k++) { char a=nm[j+k],b=query[k];
                        if(a>='A'&&a<='Z')a+=32; if(b>='A'&&b<='Z')b+=32; if(a!=b)m=0; } if(m)ok=1; } }
            if (ok) fidx[nflt++]=i;
        }
        ...
        if (ch=='\n'||ch=='\r'||ch==KEY_ENTER) {
            if (nflt>0) {
                ...
                snprintf(s_status,sizeof(s_status),
                    "Instrument: %s  (connect audio in perform.c)", s_instr);
            }
            done=1;
        }
        if (ch==27) done=1;
...
void run_performance_mode(void)
{
    ...
    snprintf(s_status, sizeof(s_status),
             "Instrument: %s  |  Up/Dn=octave  i=change instr  ESC=back", s_instr);
    ...
    while (1) {
        ...
        if (note>=0&&note<128) {
            ...
            if (!s_held[note]) {
                ...
                snprintf(s_status,sizeof(s_status),
                    "Playing: %s%d (MIDI %d)  -- TODO: connect audio engine",
                    CHROMA[note%12], note/12-1, note);
                /* TODO: audio_note_on(note, 100); */
            } else {
    ...
    for (int n=0;n<128;n++) { if(s_held[n]){ s_held[n]=0; /* TODO: audio_note_off(n); */ } }
    memset(s_held, 0, sizeof(s_held));
